...
class ProgramTidy(BaseCard):
    ...
    def play(self, player, ish, turn_mgr):
        ...
        targets = [game_state.get_player(p) for p in ish.participants
                    if p != pid and game_state.get_player(p) and game_state.get_player(p).is_alive()]
        targets += [c for c in ish.chorus_list if c.is_alive()]

        # v2.0 duet: 可上供舞台（G2/G5），但不包括自己已经在 group 中
        if ish.phase == "duet":
            g2 = game_state.get_player(ish.g2_owner_id)
            g5 = game_state.get_player(ish.duet_g5_pid) if ish.duet_g5_pid else None
            if g2 and g2.is_alive() and g2.player_id != pid:
                if not any(t.player_id == g2.player_id for t in targets):
                    targets.append(g2)
            if g5 and g5.is_alive() and g5.player_id != pid:
                if not any(t.player_id == g5.player_id for t in targets):
                    targets.append(g5)
