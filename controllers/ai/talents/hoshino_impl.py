...
class HoshinoImpl:
    ...
    def _hoshino_captain_has_police_protection(self, state) -> bool:
        ...
        pc = self._ctrl._police_cache or {}
        ...
        if not captain or not captain.is_alive():
            return False
        captain_loc = self._ctrl._get_location_str(captain)
    ...
    def _hoshino_find_target(self, player, state) -> Optional[Any]:
        ...
        # 被警察追击时优先攻击队长（搏命模式）
        if self._ctrl._is_pursued_by_police_extended(player, state):
            pc = self._ctrl._police_cache or {}
            captain_id = pc.get("captain_id")
            if captain_id:
                captain = state.get_player(captain_id)
        ...
        target = self._ctrl._pick_target(player, state)
        return target

    def _hoshino_target_same_location(self, player, target) -> bool:
        """目标是否在同一地点"""
        return self._ctrl._same_location(player, target)
    ...
    def _hoshino_pick_best_item(self, player, state, loc) -> Optional[dict]:
        ...
        if is_home:
            # 层级1：盾牌（护盾）
            if not self._ctrl._has_armor_by_name(player, "盾牌"):
            ...
            # 层级1：陶瓷护甲
            if vouchers >= 1 and not self._ctrl._has_armor_by_name(player, "陶瓷护甲"):
            ...
            if has_pass:
                # 层级1：AT力场
                if not self._ctrl._has_armor_by_name(player, "AT力场"):
    ...
    def _hoshino_best_item_destination(self, player, state) -> Optional[str]:
        ...
        if iron_horus_hp < iron_horus_max:
            # 军事基地拿AT力场（需要通行证）
            if has_pass and not self._ctrl._has_armor_by_name(player, "AT力场"):
                if loc != "军事基地":
                    return "军事基地"
            # 家拿盾牌
            if not self._ctrl._has_armor_by_name(player, "盾牌"):
                if not self._ctrl._is_at_home(player):
                    return "home"
        ...
        for dest in all_locations:
            ...
            if dest == loc:
                continue
            if dest == "home" and self._ctrl._is_at_home(player):
                continue
    ...
    def _hoshino_target_is_hard_to_kill(self, target) -> bool:
        ...
        armor_count = self._ctrl._count_outer_armor(target) + self._ctrl._count_inner_armor(target)
        if armor_count >= 2:
            return True
    ...
    def _hoshino_target_is_police_protected(self, target) -> bool:
        """目标是否受警察单体保护"""
        pe = getattr(self._ctrl._game_state, 'police_engine', None) if self._ctrl._game_state else None
        if not pe:
            return False
    ...
    def _hoshino_find_finishable_target(self, player, state) -> Optional[Any]:
        ...
        for pid in state.player_order:
            ...
            if not t or not t.is_alive():
                continue
            if not self._ctrl._same_location(player, t):
                continue
            # 无外甲 + 无内甲 + effective HP <= 1.0（一次射击 = 2发×0.5 = 1.0 伤害）
            # 或 无外甲 + 有内甲 + effective HP <= 0.5（最后内甲吸收所有溢出）
            if self._ctrl._count_outer_armor(t) > 0:
                continue
            inner_count = self._ctrl._count_inner_armor(t)
            eff_hp = self._ctrl._get_effective_hp(t)
    ...
    def _hoshino_should_use_adrenaline(self, player, target) -> bool:
        ...
        is_savior = self._ctrl._is_in_savior_state(target)
        t_talent = getattr(target, 'talent', None)
    ...
    def _hoshino_build_finish_and_switch_macro(self, player, state, finish_target, switch_target) -> List[str]:
        ...
        # 阶段2：转火到第二目标
        switch_same_loc = self._ctrl._same_location(player, switch_target)

        if not switch_same_loc:
            # 不同地点：冲刺过去 + find
            switch_loc = self._ctrl._get_location_str(switch_target)
            if can_afford("冲刺"):
    ...
    def _hoshino_build_macro(self, player, state, target) -> List[str]:
        ...
        target_loc = self._ctrl._get_location_str(target)
        ...
        if shield_mode == "架盾":
            ...
            if same_loc and has_find and prefer_deploy:
                ...
                # 持盾 + 不同地点 → 冲刺 → find
                if can_afford("冲刺"):
                    queue.append(f"冲刺 {self._ctrl._get_location_str(target)}")
                    used_cost += COST["冲刺"]
                if can_afford("find"):
                    queue.append(f"find {target.name}")
                    used_cost += COST["find"]
    ...
    def _hoshino_get_tactical_command(self, player, state, available_actions) -> str:
        """从预生成的战术宏队列中逐条弹出指令"""
        if not self._ctrl._hoshino_macro_queue:
            # 队列为空，需要生成新的
            target = self._hoshino_find_target(player, state)
            if target:
                self._ctrl._hoshino_macro_queue = self._hoshino_build_macro(player, state, target)
            else:
                self._ctrl._hoshino_macro_queue = ["terminal"]

        if self._ctrl._hoshino_macro_queue:
            cmd = self._ctrl._hoshino_macro_queue.pop(0)
