...
class DevelopCommandBuilder:
    ...
    def build_develop(
        ...
        # 天赋专用发育路径（由 TalentHook 提供，避免回调 controller 旧 mixin）
        talent_name = getattr(getattr(player, 'talent', None), 'name', '')
        if talent_hooks and talent_name:
            hook = talent_hooks.get(talent_name)
            if hook and hasattr(hook, 'get_develop_commands'):
                cmds = hook.get_develop_commands(player, state, available)
                if cmds is not None:
                    return cmds
        ...
        if "move" in available and not commands:
            next_loc = self.pick_destination(player, state, strategy, personality, ctx)
