    ...
    def _execute_debuff(self, me, round_num):
        ...
        if inner_active:
            ...
            if victim.current_hp <= 0:
                me.armor.remove_piece(victim)
                display.show_info(
                    f"🔥 血火debuff：摧毁 {me.name} 的内层护甲"
                    f"「{victim.name}」！")
            else:
                display.show_info(
                    f"🔥 血火debuff：{me.name} 的「{victim.name}」"
                    f"护甲值 {old_hp} → {victim.current_hp}")
            return

        # 没有任何护甲 → 跳过
        display.show_info(
            f"🔥 血火debuff：{me.name} 无护甲可扣，本次跳过。")
