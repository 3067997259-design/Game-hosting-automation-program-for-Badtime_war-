...
class Ripple(BaseTalent):
    ...
    def on_round_end(self, round_num):
        ...
        if self.is_anchor_paused():
            ...
            display.show_info(
                prompt_manager.get_prompt(
                    ...
                    default="🌊⏸️ 锚定倒计时因幻想乡结界暂停（剩余{remaining_rounds轮）"
        ...
        if self.was_paused_by_barrier:
            display.show_info(
                prompt_manager.get_prompt(
                    ...
                    default="🌊▶️ 结界已结束，锚定监控恢复（剩余{remaining_rounds轮）"
                ).format(remaining_rounds=self.anchor_rounds_left)
            )
        ...
        if self.anchor_type in ("acquire", "arrive"):
            display.show_info(
                prompt_manager.get_prompt(
                    ...
                    default="🌊 锚定剩余 {remaining_rounds轮（{anchor_detail）"
        ...
        display.show_info(
            ...
            f"\n" + prompt_manager.get_prompt(
                ...
                default="🌊 锚定监控 —— 剩余 {remaining_rounds轮"
            ).format(remaining_rounds=self.anchor_rounds_left)
            + "\n" + prompt_manager.get_prompt(
        ...
        if self.was_paused_by_barrier:
            display.show_info(
                prompt_manager.get_prompt(
                    ...
                    default="🌊▶️ 结界已结束，{caster_name} 的锚定监控恢复（剩余{remaining_rounds轮）"
                ).format(
    ...
    def describe_status(self):
        ...
        if self.anchor_active:
            ...
            parts.append(f"剩余{self.anchor_rounds_left轮")
            parts.append(
