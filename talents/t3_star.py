...
from combat.damage_resolver import resolve_damage, resolve_location_damage
...
class Star(BaseTalent):
    ...
    def execute_t0(self, player):
        ...
        results_dict = resolve_location_damage(
            ...
            game_state=self.state, raw_damage=1.0,
        ...
        for r in results_dict.get("players", []):
            ...
            damage_msg = prompt_manager.get_prompt("talent", "t3star.damage",
                                                  ...
                                                  target_name=t.name, old_hp=result.get("raw_damage", 1.0), new_hp=t.hp)
        ...
        for unit in results_dict.get("police", []):
            if unit.hp <= 0:
                ...
                police_damage_msg = prompt_manager.get_prompt(
                    ...
                    default="   \u2192 警察{unit_id} 受到1.0伤害+石化 HP: {new_hp}",
                    unit_id=unit.unit_id, new_hp=unit.hp)
